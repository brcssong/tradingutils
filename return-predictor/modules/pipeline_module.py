...
class Pipeline:
    ...
    def execute(self):
        ...
        if self.check_cache(FILENAMES[1], True) and self.check_cache(FILENAMES[2]) and self.check_cache(FILENAMES[3]):
            df = pd.read_pickle(f"cache/{FILENAMES[1]}")
            norm_vars = self.check_cache(FILENAMES[2])
            df_val = pd.read_pickle(f"cache/{FILENAMES[3]}")
        else:
